...
export const parseIdsFilter = ({ ids }) => (ids.length > 1 ? `any(id,${ids.reduce((acum, cur) => `${acum.length ? `${acum},` : ''}'${isObject(cur) ? cur.id : cur}'`, '')})` : `equals(id,'${isObject(ids[0]) ? ids[0].id : ids[0]}')`);
